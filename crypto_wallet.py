...
def get_balance(w3, address):
    ...
    ether = Web3.from_wei(wei_balance, "ether")  # Corrected line
...
def send_transaction(w3, account, to, wage):
    ...
    # Construct a raw transaction without the gas estimate
    raw_tx = {
        "to": to,
        "from": account.address,
        "value": value,
        "gasPrice": gas_price,
        "nonce": w3.eth.get_transaction_count(account.address),  # Fixed line
    }

    # Calculate the gas estimate
    gasEstimate = w3.eth.estimate_gas(raw_tx)

    # Update the raw transaction with the gas estimate
